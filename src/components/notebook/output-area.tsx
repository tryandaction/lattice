    ...
    <div className="rounded-md border border-destructive/40 overflow-hidden">
      ...
      <div className="flex items-start gap-2 bg-destructive/10 p-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <pre className="whitespace-pre-wrap break-words font-mono text-sm text-destructive mt-1">
            {message}
          </pre>
        </div>
      </div>
      ...
      {hasTraceback && (
        <>
          <button
            ...
            className="flex items-center gap-1 w-full px-3 py-1.5 text-xs text-muted-foreground hover:bg-muted/50 transition-colors border-t border-destructive/30"
          >
            {showTraceback ? (
              <ChevronDown className="h-3 w-3" />
            ) : (
          ...
          {showTraceback && (
            <pre className="whitespace-pre-wrap break-words font-mono text-xs text-destructive/80 bg-muted/30 p-3 border-t border-destructive/30 max-h-[300px] overflow-auto">
              {traceback.join('\n')}
            </pre>
          )}
        </>
