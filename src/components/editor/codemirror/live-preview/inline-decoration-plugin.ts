 ...
 * 
 * IMPORTANT: Cache stores elements with positions relative to line start (0-based)
 */
...
/**
 * Get cached line elements or parse fresh
 * Cache key is the line text content, positions are adjusted to actual line position
 */
function getCachedLineElements(lineText: string, lineFrom: number): MarkdownElement[] {
  ...
  if (lineParseCache.has(cacheKey)) {
    ...
    // Clone and adjust positions - cached elements have positions starting from 0
    return cached.map(el => ({
      ...el,
      from: lineFrom + el.from,
      to: lineFrom + el.to,
      syntaxFrom: lineFrom + el.syntaxFrom,
      syntaxTo: lineFrom + el.syntaxTo,
      contentFrom: lineFrom + el.contentFrom,
      contentTo: lineFrom + el.contentTo,
    }));
  }
  
  // Parse fresh with positions starting from 0 (relative to line start)
  const elements = parseLineInlineElementsInternal(lineText, 0);
  
  // Manage cache size
  if (lineParseCache.size >= MAX_CACHE_SIZE) {
    // Remove oldest entries (first 200)
    const keysToDelete = Array.from(lineParseCache.keys()).slice(0, 200);
    keysToDelete.forEach(key => lineParseCache.delete(key));
  }
  
  // Store normalized version (with position 0)
  lineParseCache.set(cacheKey, elements);
  
  // Return elements with actual positions
  ...
  for (const { from, to } of view.visibleRanges) {
    ...
    for (let lineNum = startLine.number; lineNum <= endLine.number; lineNum++) {
      ...
      for (const element of elements) {
        // Validate element positions are within line bounds
        if (element.from < line.from || element.to > line.to) {
          continue; // Skip invalid elements that cross line boundaries
        }
...
export const inlineDecorationPlugin = ViewPlugin.fromClass(
  class {
    ...
    private readonly debounceMs = 16; // ~60fps
    ...
    update(update: ViewUpdate) {
      ...
      if (update.docChanged || update.selectionSet || update.viewportChanged) {
        ...
        if (update.selectionSet && !update.docChanged) {
          ...
          // For document changes, use debounce to batch rapid updates
