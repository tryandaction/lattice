 ...
 * 
 * A comprehensive editor for editing existing text annotations with:
 * - Text editing
...
import { X, Check, Type, Palette, Minus, Plus, Trash2 } from "lucide-react";
import {
  TEXT_COLORS,
  BACKGROUND_COLORS,
  FONT_SIZES,
...
export interface TextAnnotationEditData {
  text: string;
...
interface TextAnnotationEditorProps {
  ...
  /** Position for the editor (in viewport coordinates) */
  position: { x: number; y: number };
  /** Callback when annotation is updated */
  onUpdate: (data: TextAnnotationEditData) => void;
...
export function TextAnnotationEditor({
  ...
  const [textStyle, setTextStyle] = useState<TextAnnotationStyle>(currentTextStyle);
  ...
  }, [text, backgroundColor, textStyle, onUpdate]);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-border px-3 py-2">
        <span className="text-sm font-medium">编辑文字批注</span>
        <button
          onClick={onClose}
      ...
      <div className="flex items-center justify-between border-t border-border px-3 py-2">
        <button
          onClick={onDelete}
          className="flex items-center gap-1 rounded px-3 py-1.5 text-sm text-destructive hover:bg-destructive/10"
          title="删除批注"
        >
