...
- 批注 Markdown 默认写入隐藏条目目录下的 `_annotations.md`
- PDF 首次在 Lattice 中打开时，会自动确保 `条目概览` 和 `批注索引` 已建立
- Explorer 会隐藏真实的 `.lattice` 兄弟目录，并把其中的概览、批注索引、阅读笔记、Notebook 投影到 PDF 条目下面
- `Markdown / Notebook / 批注评论` 中指向 PDF 的链接会优先在应用内打开，并支持 `#page=` 与 `#annotation=` 深链
- PDF 批注索引会自动镜像同步，并汇入“哪些笔记引用了这条批注”的反链摘要
...
- 在 Explorer 中不会直接显示这个隐藏目录，而是显示为 PDF 下的子条目：`条目概览 / 批注索引 / Reading Note / Notebook`
