...
- 设置、命令中心、帮助和插件面板等系统级按钮现在收口到左侧活动栏底部，不再和顶部文件操作混在一起
...
- 命令中心现在会优先展示 `最近使用 / 全局命令 / 插件命令` 三类结果；最近命令不会再在主列表中重复出现
...
- 对 born-digital PDF，Lattice 会优先使用 PDF.js / PDFium 提取文字模型，并把用户拖选转换为字符级文本锚点
- 复制文本、Selection AI、Evidence 引用和批注保存会优先使用 PDF 文本内核输出，不再只依赖浏览器 DOM selection 的拼接结果
- 鼠标拖拽选文会优先按实际拖拽边界裁剪首尾行，减少把拖拽范围外的同一行文字误识别进批注
- 批注会保存 PDF page-space rects/quads、quote exact/prefix/suffix、字符范围和模型版本；缩放、重开、分屏切换后会用这些信息恢复定位
- 双栏论文、公式附近文本、希腊字母、上下标、连字和引用编号已经作为重点回归场景；遇到损坏编码或异常排版 PDF 时，建议以实际复制结果检查一次
...
- 批注评论支持 Markdown、表格、代码和数学公式渲染
- 点击批注评论进入编辑时，编辑框会随内容自动增高，并允许用户继续纵向拖拽调整
- 长表格或长公式不会被评论卡片内部固定高度截断；需要横向查看时会保留横向滚动
...
- 关闭并重新打开 Lattice 后，同一个 PDF 会尽量恢复上次阅读页、滚动位置、缩放模式和侧栏状态
- 打开或关闭批注侧栏、Evidence、Workbench 等旁路面板，不应主动把 PDF 滚回第一页

### 扫描件与 OCR

- 对 born-digital PDF，Lattice 优先使用 PDF.js / PDFium 的文字解析，不会默认把清晰文本 PDF 送进 OCR
- 对扫描件或文字层过少的页面，桌面版可以使用本机 Tesseract OCR 作为免费 fallback
- 文字层很少的页面会继续允许可用的 PDF.js / PDFium 选择，同时在桌面版后台预热 OCR；纯空文字层页面不会把 DOM 选区当成可靠文字，避免选中后返回错位字符
- OCR 需要系统安装 Tesseract，并将 `tesseract` / `tesseract.exe` 加入 `PATH`；中文扫描件还需要对应语言包，例如 `chi_sim`
- OCR 结果受扫描清晰度、旋转、倾斜、语言包和公式排版影响；它用于补足可复制/可定位文本，不保证达到人工校对级准确率
...
- 直接打开这些关联文件
- 在 Explorer 中定位当前源 PDF 文件，并展开它所在的父目录
- 复制当前 PDF 的 Markdown 摘要、引用或 BibTeX

补充说明：

- 左侧 PDF 条目区现已收口为可折叠 section，不再和批注列表形成两层独立工具台
- 批注搜索、筛选、多选与导出继续保留在同一条紧凑的侧栏 chrome 中
- PDF 条目区的复制菜单会输出可直接粘贴进笔记的 Markdown 摘要，包含源 PDF 链接、路径、页数、批注数、相关文件数、DOI/arXiv 等关键信息
...
- 右侧面板支持从分隔线附近直接拖拽调整宽度，并会记住打开状态与宽度
- 关闭 AI 面板后，焦点会尽量返回到触发它的壳层按钮
- 桌面端 AI 入口统一为左侧活动栏 Bot 按钮和命令面板命令；不再通过平级 overlay 暴露 AI 上下文作为主入口
- 默认聊天会自动带入当前打开文件、当前选区和当前文件批注；如果当前文件是 PDF，会优先使用已缓存的渲染文本，否则从 PDF 文本层提取前若干页作为上下文
- 默认聊天不会隐式根据输入 query 检索整个工作区；需要跨文件材料时，继续使用显式 `@引用`、Selection AI、Evidence 操作，或在 Prompt Run 中主动勾选更重的上下文
...
- **执行作用域**：Notebook 运行状态现在绑定到 `当前 pane + 当前 tab`，同一文件在两个 pane 中互不串扰
- **切换标签/面板**：切走后不会立即销毁当前 Notebook 会话；重新切回会恢复当前 scope 的运行态、输出和问题视图
...
- **命令可信性**：`Verify / Run All / Interrupt / Restart` 都由同一份 execution state 控制，不会再保留旧 pane 或旧文件动作
- **内核切换语义**：Kernel 列表现在只负责展示候选和显式切换，不会在探测时偷偷改写当前 Notebook 内核
...
- 代码运行现在同样绑定到 `当前 pane + 当前 tab` scope；切换标签页后重新回来，仍会恢复这个 scope 的最近运行状态
- `Run / Stop / Rerun`、`Problems / Output` 和顶部 `Command Bar` 现在读取同一份 execution session，不再各自维护临时状态
- 运行失败会尽量标明具体阶段，例如：解释器发现、请求构建、会话启动、执行失败、输出解析或渲染失败
- 顶部 `Command Bar` 只消费当前 active pane + active tab 对应 scope 的动作，不会继续触发旧标签残留的运行按钮
- 代码文件命令栏提供 `Search`、`Go to Line` 和 `Outline`。Outline 会为 Markdown、Python、JavaScript、TypeScript、C/C++ 提取常见标题、类、函数和类型符号，点击后跳转到对应行
- 发布前可运行 `node scripts/code-runner-manual-smoke.mjs` 打印代码文件、Notebook 与 runner 的手工验收清单，并检查本机 Python、Node、C/C++ 工具链是否可见
...
| 拖放移动 | 将文件或文件夹拖到目标文件夹 |
| 在侧边打开 | 右键文件 → 在侧边打开 |
| 系统打开 / 定位 | 桌面版右键 → 打开方式 / 在文件资源管理器中显示 |
| 打开终端 | 桌面版右键文件或文件夹 → 在集成终端中打开 |
| 复制路径 | 右键 → 复制路径 / 复制相对路径 |
| 文件比较 | 右键文件 → 选择以进行比较，再右键另一个文件 → 与已选择文件比较；会并排打开并显示行级差异视图 |

补充说明：
- 桌面版的系统打开、Reveal 和终端入口依赖 Tauri 桌面壳；网页版会禁用这些系统级动作。
- 当前比较入口提供文本文件行级 diff；二进制文件或无法读取为文本的文件会提示比较失败。
...
- PDF 条目区的“定位”按钮会定位源 PDF 文件本身，不需要预先创建 `.lattice/items/<fileId>/` 条目目录

### PDF 重新打开后没有回到上次位置

- 确认是在同一个 workspace 中打开同一路径下的 PDF
- Lattice 会按工作区和文件路径保存阅读状态；移动、复制或重命名 PDF 后可能会被视为新文件
- 如果浏览器缓存被清空、PDF 文件损坏，或应用正在从异常状态恢复，阅读位置可能回到第一页

### 扫描 PDF 无法复制文字

- 桌面版请先安装 Tesseract OCR，并确认 `tesseract --version` 能在系统终端运行
- 中文扫描件需要安装中文语言包，并在 OCR 设置或环境中启用相应语言
- 如果页面没有可靠文字层，Lattice 会优先避免生成错误文字批注；此时仍可使用区域批注，待 OCR 可用后再进行文字复制/定位
- 网页版或未安装 OCR 时，扫描件可能只能做区域批注，无法稳定生成可复制文本
...
说明：`/diagnostics/*` 和 `/performance-test` 属于开发与发布验收直达入口，保留给回归脚本和手动 QA 使用；主工作台与命令面板默认只暴露用户指南入口，避免普通使用流程被内部工具打断。
...
- `/diagnostics/runner` 现在也会列出当前 execution sessions、lifecycle、failure stage 与最近事件，适合排查“为什么按钮能点但没有按预期执行”
