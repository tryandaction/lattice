...
桌面端主要弹窗和面板现在统一遵循更稳定的布局约束：

- 弹窗层级会覆盖标题栏，不再被顶部 Command Bar 压住
- 弹窗主体默认有最大高度限制，避免超出窗口后顶部或底部被裁切
- 内容较多时，主体区会独立滚动，而不是把底部操作区一起挤出可视范围
- 带预览区的弹窗会优先保证预览区域的最小可视高度，减少“预览只剩一小块”的问题
